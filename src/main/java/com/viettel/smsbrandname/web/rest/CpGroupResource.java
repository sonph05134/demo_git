...
/**
 * REST controller for managing {@link com.viettel.smsbrandname.domain.CpGroup}.
 */
@RestController
@RequestMapping("/api")
public class CpGroupResource {
    ...
    public CpGroupResource(CpGroupService cpGroupService) {
    ...
    public ResponseEntity<CpGroupDTO> createCpGroup(@RequestBody CpGroupDTO cpGroupDTO) throws URISyntaxException {
        ...
        return ResponseEntity.created(new URI("/api/cp-groups/" + result.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, result.getId().toString()))
            .body(result);
    }

    /**
     * {@code PUT  /cp-groups} : Updates an existing cpGroup.
     *
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PutMapping("/cp-groups")
    public ResponseEntity<CpGroupDTO> updateCpGroup(@RequestBody CpGroupDTO cpGroupDTO) throws URISyntaxException {
        log.debug("REST request to update CpGroup : {}", cpGroupDTO);
        if (cpGroupDTO.getId() == null) {
     ...
     */
    @DeleteMapping("/cp-groups/{id}/{cpId}")
    public ResponseEntity<Void> deleteCpGroup(@PathVariable Long id, @PathVariable Long cpId) {
        log.debug("REST request to delete CpGroup : {}", id);
        cpGroupService.delete(id, cpId);
        return ResponseEntity.noContent().headers(HeaderUtil.createEntityDeletionAlert(applicationName, true, ENTITY_NAME, id.toString())).build();
    }

    /**
     * {@code SEARCH  /_search/cp-groups?query=:query} : search for the cpGroup corresponding
     * to the query.
     *
     * @param query the query of the cpGroup search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search/cp-groups")
    public ResponseEntity<List<CpGroupDTO>> searchCpGroups(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of CpGroups for query {}", query);
        Page<CpGroupDTO> page = cpGroupService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
        }

    @PostMapping("/cp-groups/search")
    public ResponseEntity<List<CpGroupDetailDTO>> onSearch(@RequestBody CpGroupDTO cpGroupDTO, Pageable pageable) {
        log.debug("REST request to get a page of CpGroups");
        Page<CpGroupDetailDTO> page = cpGroupService.onSearch(cpGroupDTO, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
